...
import { NodeType } from '../../../config/config';
...
function NodeCustom({ id, data }) {
    ...
    return (
        <>
            <Handle type="target" hidden={(data.nodeType === NodeType.START)} position={Position.Top} />
            <div className="custom-node__header" style={customNodeHeaderStyle}>
                <strong>
                    {(data.nodeType !== '') ? data.nodeType : 'New Node'}
                </strong>
            </div>
            <div className="custom-node__body" hidden={(data.nodeType !== NodeType.END)}>
                <strong>Result: </strong>{((data.result === '') ? '' : data.result)}
                <br />
                <strong>Remark: </strong>{((data.remark === '') ? '' : data.remark)}
            </div>
            <div className="custom-node__body" hidden={(data.nodeType === NodeType.START || data.nodeType === NodeType.END)}>
                <strong>Node Name: </strong>{data.nodeName}
            </div>
            <Handle type="source" hidden={(data.nodeType === NodeType.END)} position={Position.Bottom} />
        </>
    );
}
